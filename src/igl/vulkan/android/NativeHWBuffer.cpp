...
Result NativeHWTextureBuffer::uploadInternal(TextureType /*type*/,
  ...
  // Release any cached CPU_READ_OFTEN lock; some drivers refuse a write lock
  // while a read lock is outstanding.
  if (cpuReadMemoryAddress_ != nullptr) {
    funcTable_->AHardwareBuffer_unlock(hwBuffer_, nullptr);
    cpuReadMemoryAddress_ = nullptr;
  }
