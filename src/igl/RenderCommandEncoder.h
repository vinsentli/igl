...
class IRenderCommandEncoder : public ICommandEncoder {
 public:
  ...
  // `attributeStride` is only honored by backends that support dynamic per-buffer attribute
  // stride (e.g. Metal with `MTLBufferLayoutStrideDynamic` on iOS 17+/macOS 14+, or Vulkan via
  // `vkCmdBindVertexBuffers2`). When `attributeStride == 0`, the stride declared in the currently
  // bound pipeline's vertex layout is used (legacy behavior).
  virtual void bindVertexBuffer(uint32_t index,
                                IBuffer& buffer,
                                size_t bufferOffset = 0,
                                size_t attributeStride = 0) = 0;
  virtual void bindIndexBuffer(IBuffer& buffer, IndexFormat format, size_t bufferOffset = 0, bool bindVAO = true) = 0;
